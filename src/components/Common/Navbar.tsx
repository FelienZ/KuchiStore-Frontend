...
export default function Navbar(){
    ...
    return(
        <NavigationMenu viewport={isMobile} className="w-screen font-[Roboto] bg-background text-foreground drop-shadow-sm border-b">
            <NavigationMenuList className="w-screen flex-wrap justify-between px-5 h-15">
                ...
                {width < 550 ? (
                    ...
                    <NavigationMenuItem className="flex items-center h-full gap-8">
                        ...
                        <NavLink to="/help" className={({isActive}) => isActive ? 'border-b-foreground border-b-3 h-full flex items-center gap-2 text-sm' : 'flex items-center gap-2 text-sm'}><HelpCircle className="size-4"/>Help</NavLink>
                    </NavigationMenuItem>
                )}
